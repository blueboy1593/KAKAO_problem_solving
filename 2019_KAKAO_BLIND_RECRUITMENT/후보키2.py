...
def solution(relation):
    ...
    def find_hubokey(kk, arr, stack):
        ...
        for k in range(kk + 1, pre_combs_leng):
            ...
            if row == len(set(tuple_arr)):
                answer_list.append(tuple(stack + [kj]))
                continue
            else:
                find_hubokey(k, hanging_arr, stack + [kj])
    ...
    for k in range(pre_combs_leng):
        ...
        # print(kj_col)
        find_hubokey(k, kj_col, [kj])

    # print(answer_list)
    
    delete_list = []
    ans_leng = len(answer_list)
    for i in range(ans_leng):
        check = answer_list[i]
        for j in range(i + 1, ans_leng):
            checked = answer_list[j]
            flag = False
            for che in check:
                if che not in checked:
                    flag = True
            if flag == False:
                if checked not in delete_list:
                    delete_list.append(checked)
    for delete in delete_list:
        answer_list.remove(delete)
